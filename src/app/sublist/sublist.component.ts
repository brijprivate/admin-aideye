...
export class SublistComponent implements OnInit {
  ...
  ngOnInit() {
    let _base = this;
    console.log("On init");
    this.http.getnewsubscriptions()
      .then(function (success: any) {
        _base.users = success.result.map((element) => {
          let subsDate = element.subscription[element.subscription.length - 1].startDate;
          let d = new Date(subsDate);
          element.subscription[element.subscription.length - 1].startDate = d.toDateString().substring(0, 15);
          return element;
        });
